...
//			PROCESS
            ...
            while ($comment = $dbComment->GetNext()) {
                $parentId = intval($comment["PARENT_ID"]);
                ...
                foreach ($comment as $key => $value) {
                    if (!in_array($key, ["POST_TEXT", "TITLE"]))
                        unset($comment["~" . $key]);
                }

                // create TREE for old-style comments. For new LIST view - we create list after, in result modifier
                if (empty($resComments[$parentId])) {

                    $resComments[$parentId] = [];

                    if ($this->arResult["firstLevel"] === '') {
                        $this->arResult["firstLevel"] = $parentId;
                    }
                }

                $resComments[$parentId][] = $comment;

                // save IDs in another array
                $this->arResult["IDS"][] = $comment["ID"];

                // save unsorted comments in another array
                $this->arResult["Comments"][$comment["ID"]] = [
                    "ID" => $comment["ID"],
            ...
            if (count($children) > 0) {
                foreach ($children as $childId => $ar) {
                    $resComments[$childId] = $ar;
                }
            }
