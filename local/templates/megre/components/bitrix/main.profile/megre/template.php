...
<section class="cabinet cabinet_inner">
    <div class="container">
		...
		<?
		$APPLICATION->IncludeFile("/local/include/personal/menu.php", array("ACTIVE" => 3), array(
			"MODE" => "html",
			"NAME" => "",
			"TEMPLATE" => ""
		));
        ...
        <div class="cabinet-container">
            <div class="page-title"><a class="link-more" href="cabinet.html">НАЗАД</a>ЛИЧНЫЕ ДАННЫЕ</div>
            <div class="cabinet-profile">
                <form class="cabinet-profile-form" action="<?=$templateFolder?>">
                    <div class="cabinet-profile-row">
                        <div class="cabinet-profile-row__title">ФИО</div>
                        <div class="cabinet-profile-row__content"><input type="text" name="FULL_NAME" value="<?echo $arResult["arUser"]["LAST_NAME"]." ".$arResult["arUser"]["NAME"]." ".$arResult["arUser"]["SECOND_NAME"];?>" readonly disabled></div>
                    </div>
                    <div class="cabinet-profile-row">
                        <div class="cabinet-profile-row__title">ТЕЛЕФОН</div>
                        <div class="cabinet-profile-row__content"><input type="text" name="PHONE_NUMBER" value="<? echo $arResult["arUser"]["PERSONAL_PHONE"]?>" readonly disabled></div>
                    </div>
                    <div class="cabinet-profile-row">
                        <div class="cabinet-profile-row__title">ЭЛ. ПОЧТА</div>
                        <div class="cabinet-profile-row__content"><input type="text" name="EMAIL" value="<?echo $arResult["arUser"]["EMAIL"]?>" readonly disabled></div>
                    </div>
                    <div class="cabinet-profile-row">
                        <div class="cabinet-profile-row__title">ГОРОД</div>
                        <div class="cabinet-profile-row__content">
                            <div class="form-item-row">
                                <div class="form-item-type-select">
                                    <select class="inputselect" name="PERSONAL_CITY" id="form_dropdown_city" disabled readonly>
                                        <option>Выбрать другой город</option>
                                        <?foreach($arResult["CITY"] as $CITY){?>
                                        <option value="<?=$CITY?>"<?if($CITY == $arResult["arUser"]["PERSONAL_CITY"])echo " selected";?>><?=$CITY?></option>
                                        <?}?>
...
<!--                            <input type="text" name="PERSONAL_CITY" value="--><?//echo $arResult["arUser"]["PERSONAL_CITY"]?><!--" readonly disabled>-->
                        </div>
                    </div>
                    <div class="cabinet-profile-row">
                        <div class="cabinet-profile-row__title">ДАТА РОЖДЕНИЯ</div>
                        <div class="cabinet-profile-row__content"><input type="text" name="PERSONAL_BIRTHDAY" value="<?echo $arResult["arUser"]["PERSONAL_BIRTHDAY"]?>" readonly disabled></div>
                    </div>
                    <div class="cabinet-edit-row" style="display: none;">
                        <div class="cabinet-edit-row__title">Изменить пароль</div>
                    </div>
                    <div class="cabinet-profile-row">
                        <div class="cabinet-profile-row__title password-label">ПАРОЛЬ</div>
                        <div class="cabinet-profile-row__content password__box">Скрыто</div>
                    </div>
                </form>
                <div class="button button_primary edit_profile">Редактировать данные</div>
            </div>
        </div>
    </div>
</section>
...
<script>
    $(function(){
        $('body').on('click', '.edit_profile', function(){
            $('.cabinet-profile-form').find('input').removeAttr('readonly disabled');
            $('.cabinet-profile-form').find('select').removeAttr('readonly disabled');
            $('.cabinet-profile-form').next().toggleClass('edit_profile save_profile').text('Сохранить');
            $('.password__box').html('<input type="password" name="PASSWORD" value=""><div class="icon-eye"></div>');
        });

        $('body').on('click', '.save_profile', function(){
            let data = $('.cabinet-profile-form').serialize();
            let action = $('.cabinet-profile-form').attr('action');
            $.ajax({
                type: "POST",
                url: action + "/edit.php",
                data: data
            ...
            $('.cabinet-profile-form').find('input').attr('readonly','readonly').attr('disabled','disabled');
            $('.cabinet-profile-form').find('select').attr('readonly','readonly').attr('disabled','disabled');
            $('.cabinet-profile-form').next().toggleClass('edit_profile save_profile').text('Редактировать данные');
            $('.password__box').html('Скрыто');
        });
    })
</script>
<?php
