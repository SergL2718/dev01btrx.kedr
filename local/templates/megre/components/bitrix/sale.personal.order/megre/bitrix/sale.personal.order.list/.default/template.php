...
<section class="cabinet cabinet_inner">
    <div class="container">
		...
		<? $APPLICATION->IncludeFile("/local/include/personal/menu.php", array("ACTIVE" => 1), array(
			"MODE" => "html",
			"NAME" => "",
        ...
        <div class="cabinet-container">
            <div class="page-title"><a class="link-more" href="/personal/">НАЗАД</a>ЗАКАЗЫ</div>
            <div class="cabinet-order-list">
				...
				<? if (!count($arResult['ORDERS'])) {
                    ...
                    <?foreach($arResult['ORDERS'] as $ORDER){?>
                        <div class="cabinet-order-card">
                            <div class="cabinet-order-card__header">
                                <p>№ <span><?=$ORDER["ORDER"]["ID"]?></span> от <span><?=$ORDER["ORDER"]["DATE_INSERT_FORMATED"]?></span></p>
                                <a class="link-more" href="<?=$ORDER["ORDER"]["URL_TO_DETAIL"]?>">ПОДРОБНЕЕ</a>
                            </div>
                            <div class="cabinet-order-card__body">
                                <div class="cabinet-order-card__action">
                                    <div class="cabinet-order-card__action-title"><?=$ORDER["SHIPMENT"][0]["DELIVERY_NAME"]?></div>
                                    <div class="cabinet-order-card__action-status">
                                        <div class="button button_outline"><?
                                ...
                                <div class="cabinet-order-card__products">
                                    <?foreach($ORDER["BASKET_ITEMS"] as $BASKET_ITEMS){
										$res = CIBlockElement::GetByID($BASKET_ITEMS["PRODUCT_ID"]);
										if($ar_res = $res->GetNext()){
											$PREVIEW_PICTURE = CFile::GetPath($ar_res["DETAIL_PICTURE"]);
                                            ?><img src="<?=$PREVIEW_PICTURE?>" alt="<?=$BASKET_ITEMS["NAME"]?>"><?
                                        }
                                        ?>
                                    <?}?>
                                </div>
                                <div class="cabinet-order-card__price">
                                    <b><?=$ORDER["ORDER"]["FORMATED_PRICE"]?></b><span><?echo ($ORDER["ORDER"]["PAYED"]=="Y")?"Оплачен":"Не оплачен";?></span>
                                </div>
                            </div>
                            <div class="cabinet-order-card__footer">
                                <a class="button button_outline" href="<?=$ORDER["ORDER"]["URL_TO_DETAIL"]?>">Подробнее</a>
                            </div>
                        </div>
                    <?}?>
