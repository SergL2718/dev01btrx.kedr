...
<div id="<?= $idCompareCount; ?>" class="<?= $mainClass; ?> "<?= $style; ?>><?
	...
	if ($itemCount > 0) {
		...
		<p><?= GetMessage('CP_BCCL_TPL_MESS_COMPARE_COUNT'); ?>&nbsp;<span data-block="count"><?= $itemCount; ?></span>
		</p>
		<p class="compare-redirect">
			<a href="<?= $arParams["COMPARE_URL"]; ?>"><?= GetMessage('CP_BCCL_TPL_MESS_COMPARE_PAGE'); ?></a></p>
		</div>
		...
		<table class="compare-items" data-block="item-list">
			...
			foreach ($arResult as $arElement) {
				...
				<td>
					<noindex><a href="javascript:void(0);"
