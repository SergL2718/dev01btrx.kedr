...
$arParams['~CONTAINER_ID'] = trim($arParams['~CONTAINER_ID']);
if (!$arParams['~CONTAINER_ID']) {
	$arParams['~CONTAINER_ID'] = 'title-search';
}
$arParams['~INPUT_ID'] = trim($arParams['~INPUT_ID']);
	...
	if ($arItem["MODULE_ID"] == "iblock") {
		if (array_key_exists($arItem["ITEM_ID"], $arResult["ELEMENTS"])) {
			...
			if ($arParams["SHOW_PREVIEW"] == "Y") {
				...
				else if ($arElement["DETAIL_PICTURE"] > 0)
