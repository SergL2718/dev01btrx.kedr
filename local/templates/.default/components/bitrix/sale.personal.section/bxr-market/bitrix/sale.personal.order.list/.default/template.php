...
if (!empty($arResult['ERRORS']['FATAL'])) {
    ...
    <div class="row col-md-12 col-sm-12">
        ...
        if ($nothing || $_REQUEST["filter_history"] == 'N') {
            ?>
            <a class="sale-order-history-link bxr-font-color bxr-border-color"
    ...
    if (!count($arResult['ORDERS'])) {
        ...
        <div class="row col-md-12 col-sm-12">
            <a href="<?= htmlspecialcharsbx($arParams['PATH_TO_CATALOG']) ?>"
               class="sale-order-history-link bxr-font-color bxr-border-color">
                                    ...
                                    if ($payment['PAID'] === 'N' && $payment['IS_CASH'] !== 'Y') {
                                        if ($order['ORDER']['IS_ALLOW_PAY'] == 'N') {
                                            ...
                                            <div class="col-md-3 col-sm-4 col-xs-12 sale-order-list-button-container">
                                                ...
                                                <? //='<pre>'.print_r($payment['PAY_SYSTEM_ID'], 1).'</pre>'
                                                ?>
                                            </div>
                                            <?
                                        } else {
                        ...
                        <div class="row sale-order-list-more-info">
                            <div class="sale-order-list-about-container">
                                <a class="sale-order-list-about-link"
                                   href="/personal/order/?ID=<?= htmlspecialcharsbx($order["ORDER"]["ID"]) ?>"><?= Loc::getMessage('SPOL_TPL_MORE_ON_ORDER') ?></a>
                            </div>
                            <div class="sale-order-list-cancel-container">
                                <a class="sale-order-list-cancel-link"
                                   href="<?= htmlspecialcharsbx($order["ORDER"]["URL_TO_CANCEL"]) ?>"><i
                                            class="fa fa-times"
                                            aria-hidden="true"></i><?= Loc::getMessage('SPOL_TPL_CANCEL_ORDER') ?></a>
                            </div>
                            <div class="sale-order-list-repeat-container">
                                <a class="sale-order-list-repeat-link"
                                   href="<?= htmlspecialcharsbx($order["ORDER"]["URL_TO_COPY"]) ?>"><i
                                            class="fa fa-undo"
                                            aria-hidden="true"></i><?= Loc::getMessage('SPOL_TPL_REPEAT_ORDER') ?></a>
                            </div>
                        </div>
                    </div>
                </div>
