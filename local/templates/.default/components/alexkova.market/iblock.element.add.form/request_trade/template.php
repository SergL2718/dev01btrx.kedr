...
<form id="iblockForm<?= $arParams["IBLOCK_ID"] ?>" name="iblock_add" action="<?= POST_FORM_ACTION_URI ?>" method="post"
    ...
    <? if (is_array($arResult["PROPERTY_LIST"]) && !empty($arResult["PROPERTY_LIST"])): ?>
                ...
                case "N":
                    for ($i = 0; $i < $inputNum; $i++) {
                        ...
                        if ($inputType != 'textarea') { ?>
                            <input type="<?= $inputType ?>" name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" size="25"
                                   value="<?= $value ?>"
                            ...
                            <? } ?>
                        <? } else { ?>
                            <textarea name="PROPERTY[<?= $propertyID ?>][<?= $i ?>]" rows="5"
                                      data-code="<?= $arResult["PROPERTY_LIST_FULL"][$propertyID]["CODE"] ?>"
                                      style="resize: none"><?= $value ?></textarea><br/><br/>
                            <?
                        } ?>
                        <?
        ...
        <? if ($arResult['USE_PERSONAL_ACCEPT']): ?>
            <br/>
            <input type="checkbox" name="accept_personal" value="yes"
                   id="accept_form_<?= $arParams["IBLOCK_ID"] ?>_<?= $BXR_FORM_COUNTER ?>" checked>
            <label for="accept_form_<?= $arParams["IBLOCK_ID"] ?>_<?= $BXR_FORM_COUNTER ?>" class="accept-form">
                <?= $arResult['PERSONAL_SETTINGS']['CAPTION'] ?> <? if ($arResult['PERSONAL_SETTINGS']['ACCEPT_DOCUMENT'] && $arResult['PERSONAL_SETTINGS']['URL']) { ?>[
                    <a href="<?= $arResult['PERSONAL_SETTINGS']['URL'] ?>"
                       target="_blank"><?= $arResult['PERSONAL_SETTINGS']['ACCEPT_DOCUMENT'] ?>
                    </a>]<? } ?>
    ...
    <input onclick="ga('send', 'event', 'buy', 'oneClickSend'); yaCounter47420761.reachGoal('oneClickSend');BXReady.Market.formRefresh(<?= $arParams["IBLOCK_ID"] ?>);return false;" <? if ($arParams["HIDE_SUBMIT"] == 'Y') echo 'style="display: none;"' ?>
           class="color-button bxr-color bxr-color-button bxr-bg-hover-light"
           id="submitForm_<?= $arParams["IBLOCK_ID"] ?>" type="submit" name="iblock_submit"
