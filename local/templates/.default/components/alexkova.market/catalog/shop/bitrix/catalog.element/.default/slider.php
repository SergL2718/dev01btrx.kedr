            ...
            <? if (count($arResult["MORE_PHOTO"]) > 0 && $arResult["MORE_PHOTO"][0]): ?>
                <? foreach ($arResult["MORE_PHOTO"] as $key => $val):
                    ...
                    <a href="<?= $val["SRC"] ?>" class="fancybox"
                       rel="product-gallery" <? if ($key == 0) echo 'id="main-photo"' ?>>
                        <img src="<?= $val["SRC"] ?>" class="zoom-img" title="<?= $imgTitle ?>" alt="<?= $imgAlt ?>"
                             data-state="show" data-large="<?= $val["SRC"] ?>" data-text-bottom="<?= $imgTitle ?>"
                             itemprop="image">
                    </a>
                <? endforeach; ?>
                ...
                <? foreach ($arResult["MORE_PHOTO"] as $key => $val):
                    ...
                    <div>
                        <div class="slide-wrap <? if ($key == 0) echo 'first-slide' ?>"
                             data-item="<?= $val["ITEM_ID"] ?>" <? if ($key == 0) echo 'id="main-photo-small"' ?>>
                            <img src="<?= $val["SRC"] ?>" title="<?= $imgTitle ?>" alt="<?= $imgAlt ?>"
                                 itemprop="image">
                        </div>
                    </div>
                <? endforeach; ?>
