<?php
/*
 * Изменено: 31 мая 2021, понедельник
...
if (strlen($arResult['PROPERTIES']['REDIRECT_LINK']['VALUE']) > 0) {
    LocalRedirect(trim($arResult['PROPERTIES']['REDIRECT_LINK']['VALUE']));
    die;
}

// https://megre.bitrix24.ru/workgroups/group/9/tasks/task/view/21639/
// Етекущий товар является дублирующим друго товар
// Тогда редиректим текущий товар на дублируемые
if (!empty($arResult['PROPERTIES']['DUPLICATED']['VALUE'])) {
    $duplicatedElementId =& $arResult['PROPERTIES']['DUPLICATED'];
    if ($duplicatedElementUrl = \CIBlockElement::GetList([], ['=ID' => $duplicatedElementId], false, false, ['DETAIL_PAGE_URL'])->GetNext()['DETAIL_PAGE_URL']) {
        LocalRedirect($duplicatedElementUrl);
        die;
    }
}
...
if ($arResult['PROPERTIES']['HIDDEN']['VALUE_XML_ID'] === 'Y') {
...
if ($arResult['CATALOG_CAN_BUY_ZERO'] === 'Y' && $arResult['CATALOG_QUANTITY'] <= 0) {
    $arResult['CATALOG_QUANTITY'] = 9999999;
}
...
if ($arResult['MODULES']['catalog']) {
    ...
    switch ($arResult['CATALOG_TYPE']) {
        case CCatalogProduct::TYPE_SET:
            ...
            $arResult['CATALOG_MEASURE_RATIO'] = 1;
            // http://task.anastasia.ru/issues/3650
            // Для комплекта всегда отображалось Нет в наличии
            //$arResult['CATALOG_QUANTITY'] = 0;
            $arResult['CHECK_QUANTITY'] = false;
            break;
...
if ($arResult['PROPERTIES']['DOWNLOAD_LINK']['VALUE']) {
    $arResult['CHECK_QUANTITY'] = false; // не проверяем доступность
    ...
    $arResult['HAS_ACCESS_TO_PRODUCT'] = $r->getSelectedRowsCount() > 0;
}
...
if ($arResult['CATALOG'] && isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])) {
    ...
    foreach ($arResult['OFFERS'] as &$arOffer) {
        ...
        if ($arOffer['PROPERTIES']['DOWNLOAD_LINK']['VALUE'] || $arResult['PROPERTIES']['DOWNLOAD_LINK']['VALUE']) {
            $arOffer['CHECK_QUANTITY'] = false; // не проверяем доступность
    ...
    unset($arOfferIds, $arOffer, $accessOfferList);
...
if ($arResult['PROPERTIES']['DOWNLOAD_LINK']['VALUE']) {
    $arResult['CATALOG_QUANTITY'] = 99999999;
    $arResult['PRODUCT']['QUANTITY'] = 99999999;
} else {
    $quantityInStore = Product::getInstance()->getQuantityProductsByStoreCode($arResult['ID'], Location::MSK);
    if (isset($quantityInStore[$arResult['ID']])) {
        $quantityInStore = $quantityInStore[$arResult['ID']];
        if (Location::getCurrentCityCode() === Location::MSK) {
            $arResult['CATALOG_QUANTITY'] = $quantityInStore;
            $arResult['PRODUCT']['QUANTITY'] = $quantityInStore;
        } else if ($quantityInStore > -1) {
