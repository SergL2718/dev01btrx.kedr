                ...
                <div class="bxr-market-item-price-group">
                    <? if (count($arResult["PRICE_MATRIX"]["ROWS"]) > 1) { ?>
                        <div class="bxr-market-item-range-title arrow">
                            <?= GetMessage("BXR_DETAIL_BUY_RANGE") . ' ' . $itemRange["QUANTITY_FROM"];
                            if ($itemRange["QUANTITY_TO"] > 0) echo '-' . $itemRange["QUANTITY_TO"] ?>
                        ...
                        <? if (in_array($priceIndex, $arResult["PRICES_ALLOW"])): ?>
                            <div class="bxr-market-item-price bxr-format-price
                                ...
                                <span class="bxr-market-current-price bxr-market-format-price"
                                      id="<? echo $arItemIDs['PRICE']; ?>"><?= CurrencyFormat($price[$itemRangeKey]['DISCOUNT_PRICE'], $price[$itemRangeKey]['CURRENCY']) ?></span>
                                <meta itemprop="price"
                                      content="<?= ($price[$itemRangeKey]['DISCOUNT_PRICE']) ? $price[$itemRangeKey]['DISCOUNT_PRICE'] : 0 ?>">
                                <meta itemprop="priceCurrency"
                                      content="<?= ($price[$itemRangeKey]['CURRENCY']) ? $price[$itemRangeKey]['CURRENCY'] : 'RUB' ?>">
                                <!--old price-->
                                <? if ($boolDiscountShow && $price[$itemRangeKey]['PRICE'] != $price[$itemRangeKey]['DISCOUNT_PRICE']) { ?>
                                    <span class="bxr-market-old-price"
                                          id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?= CurrencyFormat($price[$itemRangeKey]['PRICE'], $price[$itemRangeKey]['CURRENCY']) ?></span>
                                <? } ?>
                        ...
                        <? foreach ($price as $priceRangeIndex => $priceRange) { ?>
                            <div class="bxr-market-item-price bxr-format-price <? if (count($arResult["PRICE_MATRIX"]["ROWS"]) == 1) echo "bxr-no-range-price" ?>
                                ...
                                <span class="bxr-market-price-name">
                                        <? if (count($arResult["PRICE_MATRIX"]["ROWS"]) > 1) { ?>
                                            <?= GetMessage("BXR_DETAIL_BUY_RANGE") . ' ' . $arResult["PRICE_MATRIX"]["ROWS"][$priceRangeIndex]["QUANTITY_FROM"];
                                            if ($arResult["PRICE_MATRIX"]["ROWS"][$priceRangeIndex]["QUANTITY_TO"] > 0) echo '-' . $arResult["PRICE_MATRIX"]["ROWS"][$priceRangeIndex]["QUANTITY_TO"] ?>
                                            <?= ' ' . $arResult["CATALOG_MEASURE_NAME"] ?>
                                ...
                                <!--current price with all discounts-->
                                <span class="bxr-market-current-price bxr-market-format-price"
                                      id="<? echo $arItemIDs['PRICE']; ?>"><?= CurrencyFormat($priceRange['DISCOUNT_PRICE'], $priceRange['CURRENCY']) ?></span>
                                <meta itemprop="price"
                                      content="<?= ($priceRange['DISCOUNT_PRICE']) ? $priceRange['DISCOUNT_PRICE'] : 0 ?>">
                                <meta itemprop="priceCurrency"
                                      content="<?= ($priceRange['CURRENCY']) ? $priceRange['CURRENCY'] : 'RUB' ?>">
                                <!--old price-->
                                <? if ($boolDiscountShow && $priceRange['PRICE'] != $priceRange['DISCOUNT_PRICE']) { ?>
                                    <span class="bxr-market-old-price"
                                          id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?= CurrencyFormat($priceRange['PRICE'], $priceRange['CURRENCY']) ?></span>
                                <? } ?>
                                <div class="clearfix"></div>
                            </div>
                        <? } ?>
        ...
        <? foreach ($arResult["PRICES"] as $priceCode => $arPrice): ?>
            <? if (in_array($arResult["CAT_PRICES"][$priceCode]["ID"], $arResult["PRICES_ALLOW"])): ?>
                <div class="bxr-market-item-price bxr-format-price <? if (!$priceNameShow || count($arResult["PRICES"]) == 1) echo 'bxr-market-price-without-name' ?>">
                    ...
                    <meta itemprop="price"
                          content="<?= ($arPrice['DISCOUNT_VALUE']) ? $arPrice['DISCOUNT_VALUE'] : 0 ?>">
                    <meta itemprop="priceCurrency"
                          content="<?= ($arPrice['CURRENCY']) ? $arPrice['CURRENCY'] : 'RUB' ?>">
                    <!--old price-->
                    <? if ($boolDiscountShow && $arPrice['VALUE'] != $arPrice['DISCOUNT_VALUE']) { ?>
                        <span class="bxr-market-old-price"
                              id="<? echo $arItemIDs['OLD_PRICE']; ?>"><?= $arPrice['PRINT_VALUE'] ?></span>
                    <? } ?>
                    <div class="clearfix"></div>
                </div>
                <? if (!$priceNameShow || count($arResult["PRICES"]) == 1) { ?>
        ...
        <? foreach ($arResult["MIN_PRICE"] as $priceCode => $arPrice): ?>
            <? if (in_array($arResult["CAT_PRICES"][$priceCode]["ID"], $arResult["PRICES_ALLOW"])): ?>
                <div class="bxr-market-item-price bxr-format-price <? if (!$priceNameShow || count($arResult["MIN_PRICE"]) == 1) echo 'bxr-market-price-without-name' ?>">
                    ...
                    <!--current price with all discounts-->
                    <span class="bxr-market-current-price bxr-market-format-price"
                          id="<? echo $arItemIDs['PRICE']; ?>"><?= CurrencyFormat($arPrice['DISCOUNT_VALUE'], $arPrice["CURRENCY"]) ?></span>
                    <? if ($showFrom) { ?>
