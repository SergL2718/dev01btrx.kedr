...
if ($arParams["OFFERS_VIEW"] == "CHOISE") { ?>
    <div class="sku-choise-wrap">
        <? foreach ($arResult["SKU_PROPS_LIST"] as $code => $prop) { ?>
            <div class="sku-prop" data-prop-id="<?= $prop["ID"] ?>" data-prop-code="<?= $prop["CODE"] ?>"
                 data-prop-name="<?= $prop["NAME"] ?>" data-prop-type="<?= $prop["PROPERTY_TYPE"] ?>">
                ...
                <ul class="sku-prop-values-list">
                    <? foreach ($prop["VALUES"] as $valId => $propVal) {
                        ...
                        <li class="sku-prop-value <?= $valueType ?> <?= $addClass ?>" data-prop-code="<?= $code ?>"
                            data-prop-type="<?= $prop["PROPERTY_TYPE"] ?>" data-val-id="<?= $propVal["ID"] ?>"
                            data-val-code="<?= $propVal["XML_ID"] ?>" data-val-name="<?= $propVal["NAME"] ?>">
                            <? if ($propVal["FILE"]) { ?>
