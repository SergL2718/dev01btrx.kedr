    ...
    <div class="col-lg-5 col-md-5 col-sm-12 col-xs-12">
        ...
        <?php if ($arResult['PROPERTIES']['CHANNEL_SALE']['VALUE_XML_ID'] === \Native\App\Catalog\Product::TYPE_RETAIL): ?>
            <div class="only-online-store-notification">
                ТОВАР ДОСТУПЕН ТОЛЬКО В МАГАЗИНЕ «МЕГРЕ» в Новосибирске
            </div>
        <?php endif ?>
        ...
        if ($useVoteRating) {
            ?>
            <div class="bxr-rating-wrap" itemprop="aggregateRating" itemscope
                 itemtype="http://schema.org/AggregateRating">
                <meta itemprop="ratingValue"
                      content="<?= ($arResult["PROPERTIES"]["rating"]["VALUE"]) ? $arResult["PROPERTIES"]["rating"]["VALUE"] : 0 ?>">
                <meta itemprop="ratingCount"
                      content="<?= ($arResult["PROPERTIES"]["vote_count"]["VALUE"]) ? $arResult["PROPERTIES"]["vote_count"]["VALUE"] : '0' ?>">
        ...
        <div id="bxr-market-price-wrap" itemprop="offers"
             itemscope <?= (count($arResult["OFFERS"]) > 0) ? 'itemtype="http://schema.org/AggregateOffer"' : 'itemtype="http://schema.org/Offer"' ?>>
            <? include_once 'prices.php'; ?>
        </div>
        <!--sku-select block -->
        <? if (count($arResult["OFFERS"]) > 0) { ?>
            <div id="bxr-market-sku-select-wrap">
        ...
        <? if ($arResult["PREVIEW_TEXT"] || $arParams["PREVIEW_DETAIL_PROPERTY_CODE"][0]) { ?>
