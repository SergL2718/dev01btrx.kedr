...
if ($arCurSection) {
    // CHAIN
    $res = CIBlockSection::GetNavChain($arParams["IBLOCK_ID"], $arCurSection["ID"], ["ID", "NAME", "SECTION_PAGE_URL"]);
    while ($ar_section = $res->GetNext()) {
        $APPLICATION->AddChainItem($ar_section['NAME'], $ar_section['SECTION_PAGE_URL']);
    }

    // SEO
    $props = new \Bitrix\Iblock\InheritedProperty\SectionValues($arParams["IBLOCK_ID"], $arCurSection['ID']);
    $arCurSection['SEO'] = $props->getValues();
}
?>
...
<div class="<?= (($isFilter || $isSidebar) ? "col-md-9 col-sm-8" : "col-xs-12") ?>">
    <div class="section-title"><?= $arCurSection['NAME'] ?></div>
    <div class="row">
        ...
        <div class="col-xs-12">
            ...
            $GLOBALS[$arParams['FILTER_NAME']]['PROPERTY']['HIDDEN'] = false;
...
//			"OFFERS_LIMIT" => $arParams["LIST_OFFERS_LIMIT"],
        ...
        <? if ($arCurSection['SEO']['SECTION_PAGE_TITLE']): ?>
            <h1 style=" padding-left: 10px;"><?= $arCurSection['SEO']['SECTION_PAGE_TITLE'] ?></h1>
        <? endif ?>

        <?
        if ($arCurSection["DESCRIPTION"] && $arParams["SHOW_SECTION_DESC"] == "bottom") { ?>
            <div class="bxr-section-desc">
                <? if ($_GET['PAGEN_1'] || $_GET['SHOWALL_1']): ?><!--noindex--><? endif; ?>
                <?= $arCurSection["DESCRIPTION"] ?>
                <? if ($_GET['PAGEN_1'] || $_GET['SHOWALL_1']): ?><!--/noindex--><? endif; ?>
            </div>
        <? } ?>
...
if ($_GET['PAGEN_1']) {
    ...
    $APPLICATION->SetPageProperty('description', 'Страница: ' . $_GET['PAGEN_1']);
}
if ($_GET['SHOWALL_1']) {
    $APPLICATION->SetTitle($APPLICATION->GetTitle() . ' | Все страницы');
    $APPLICATION->SetPageProperty('description', 'Все страницы');
}
?>
