...
elseif ($obCache->StartDataCache())
{
        ...
        {
                $dbRes = CIBlockSection::GetList(array(), $arFilter, false, array("ID", "NAME", "DESCRIPTION", "PICTURE"));
                ...
                if(defined("BX_COMP_MANAGED_CACHE"))
                {
                        ...
                        {
                            $arCurSection['PICTURE'] = CFile::GetPath($arCurSection['PICTURE']);
...
<h1><?$APPLICATION->ShowTitle(false);?></h1>
	...
	//echo '<pre>'.print_r($arCurSection, 1).'</pre>';
	?>
        <?if ($arCurSection["DESCRIPTION"] && $arParams["SHOW_SECTION_DESC"] == "top"):?>
            <div class="row">
                <div class="col-xs-12 tb20">
                    <?if ($arCurSection["PICTURE"]):?>
                        <img src="<?=$arCurSection["PICTURE"]?>" align="right" style="margin-left: 20px;margin-bottom: 20px;">
                    <?endif;?>
                    <div class="bxr-section-desc" style="text-align: justify"><?=$arCurSection["DESCRIPTION"]?></div>
                </div>
            </div> 
        <?endif;?>
