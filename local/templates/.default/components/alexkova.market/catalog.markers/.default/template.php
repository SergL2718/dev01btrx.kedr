...
<div id="bxr-markers-container">
    <? if (strlen($arParams["BXREADY_LIST_PAGE_BLOCK_TITLE"]) > 0): ?>
        <h2 class="bxr-font-color"><?
            if (!empty($arParams["BXREADY_LIST_PAGE_BLOCK_TITLE_GLYPHICON"])) echo "<i class='fa " . $arParams["BXREADY_LIST_PAGE_BLOCK_TITLE_GLYPHICON"] . "'></i>"; ?><?= $arParams["BXREADY_LIST_PAGE_BLOCK_TITLE"] ?></h2>
    <?endif;
        ...
        <div class="col-lg-12 lent">
            <? foreach ($arResult["MARKERS_LIST"] as $cell):
                switch ($cell) {
                    case "RECCOMEND":
                        ?>
                        <div class="bxr-markers-group" id="marc_tabRECOMMENDED" data-slide="RECOMMENDED"
                             data-type="markers"><?= GetMessage('RECOMMENDED_BUTTON') ?></div>
                        <?
                        if (!$firstMarker) $firstMarker = "RECOMMENDED";
                        break;
                    case "NEW":
                        ?>
                        <div class="bxr-markers-group" id="marc_tabNEWPRODUCT" data-slide="NEWPRODUCT"
                             data-type="markers"><?= GetMessage('NEWPRODUCT_BUTTON') ?></div>
                        <?
                        if (!$firstMarker) $firstMarker = "NEWPRODUCT";
                        break;
                    case "ACTION":
                        ?>
                        <div class="bxr-markers-group" id="marc_tabSPECIALOFFER" data-slide="SPECIALOFFER"
                             data-type="markers"><?= GetMessage('SPECIALOFFER_BUTTON') ?></div>
                        <?
                        if (!$firstMarker) $firstMarker = "SPECIALOFFER";
                        break;
                    case "HIT":
                        ?>
                        <div class="bxr-markers-group" id="marc_tabSALELEADER" data-slide="SALELEADER"
                             data-type="markers"><?= GetMessage('SALELEADER_BUTTON') ?></div>
                        <?
