...
<? if (!empty($arResult["TREE"])): ?>
    ...
    <div class="row">
        <div class="col-sm-12">
            <nav>
                <ul data-style-menu="<?= $arParams['STYLE_MENU'] ?>"
                    data-style-menu-hover="<?= $arParams['STYLE_MENU_HOVER'] ?>"
                    class="bxr-flex-menu  <?= $classUl; ?> bxr-top-menu">
                    <?
                    $previousLevel = 0;
                    $flagFirst = true;
                    $i = 0;
                    ...
                    foreach ($arResult["TREE"] as $arItem):?>
                        ...
                        if (trim($arItem['LINK'], '/') == 'cooperation') {
                            $classLi .= ' custom-submenu';
                        } else {
                            $classLi = str_replace(' custom-submenu', '', $classLi);
                        }
                        ?>
                        <li class="<?= $classLi . " " . $classLiParent; ?> <? if ($arItem['SELECTED'] == 1) echo $classLiSelected; ?>">
                            ...
                            <? if ($isChildren): ?>
                                ...
                                if (isset($arParams["TEMPLATE_MENU_HOVER"]) && $arParams["TEMPLATE_MENU_HOVER"] == "list") {
                                    ...
                                    if (trim($arItem['LINK'], '/') == 'cooperation' || trim($arItem['LINK'], '/') == 'articles') {
                                        $column = 3;
                                        $arParamsHoverMenu["HOVER_MENU_COL_LG"] = $column;
                                        $arParamsHoverMenu["HOVER_MENU_COL_MD"] = $column;
                                        $arParamsHoverMenu["HOVER_MENU_COL_SM"] = $column;
                                        $arParamsHoverMenu["HOVER_MENU_COL_XS"] = $column;
                                    } else {
                    ...
                    <? if ($arParams['SEARCH_FORM'] == "Y"): ?>
                        ...
                        <li class="last li-visible <?= $classLi; ?>"><a href="#"><span class="fa fa-search"></span></a>
                        </li>
                    <? else: ?>
...
<? //endif;?>
<div class="">
    <div class="hidden-lg hidden-md bxr-mobile-menu-button-container">
        <div class="bxr-color-flat">
            <div class="bxr-mobile-menu-text"><? if (isset($arParams["TEXT_MOBIL_MENU"])) echo $arParams["TEXT_MOBIL_MENU"]; ?></div>
            <div id="bxr-menuitem" class="bxr-mobile-menu-button pull-right"><i class="fa fa-bars"></i></div>
            <? if ($arParams['SEARCH_FORM'] == "Y"): ?>
                <div id="bxr-menu-search-form" class="bxr-mobile-menu-button pull-right"><i
    ...
    <? endif; ?>
    <div class="hidden-lg hidden-md" id="bxr-mobile-menu-container">
