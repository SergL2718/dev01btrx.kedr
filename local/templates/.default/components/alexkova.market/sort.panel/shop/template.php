    ...
    <div class="row">
        <div class="col-xs-12 col-sm-12 col-md-5 text-xs-right text-md-left">
            ...
            <? foreach ($arResult["SORT_PROPS"] as $key => $val):
                ...
                <a href="<?= $APPLICATION->GetCurPageParam('sort=' . $key . '&order=' . $newSort, array('sort', 'order')) ?>"
                   class="bxr-sortbutton<?= $className ?> <? if (number_key($arResult["SORT_PROPS"], $key) == count($arResult["SORT_PROPS"])) echo "last"; ?>"
                   rel="nofollow">
        ...
        <? if ($numShow): ?>
            <div class="col-xs-12 col-sm-<?= $viewShow ? '6' : '12' ?> col-md-<?= $viewShow ? '4' : '7' ?> text-right">
                ...
                <? foreach ($arParams["PAGE_ELEMENT_COUNT_LIST"] as $val) : ?>
                    <? if ($val > 0): ?>
                        <a href="<?= $APPLICATION->GetCurPageParam('num=' . $val, array('num')); ?>"
                           title="<?= GetMessage('KZNC_VIEW_BY') . " " . $val . " " . GetMessage('KZNC_ITEM_NAME') . NumberWordEndingsEx($val) ?>"
                           class="bxr-view-mode<?= ($arSortGlobal["num"] == $val) ? ' active' : ''; ?>">
                            <?= $val; ?>
                        </a>
                    <? endif; ?>
                <? endforeach; ?>
        ...
        <? if ($viewShow): ?>
            <div class="col-xs-12 col-sm-<?= $numShow ? '6' : '12' ?> col-md-<?= $numShow ? '3' : '7' ?> text-right">
                ...
                <a href="<?= $APPLICATION->GetCurPageParam('view=title', array('view')); ?>"
                   title="<?= GetMessage('KZNC_VIEW_PLITKA') ?>"
                   class="bxr-view-mode<?= ($arSortGlobal['view'] == 'title' || !$arSortGlobal['view']) ? ' active' : ''; ?>">
                    <i class="fa fa-th"></i>
                </a>
                <a href="<?= $APPLICATION->GetCurPageParam('view=list', array('view')); ?>"
                   title="<?= GetMessage('KZNC_VIEW_LIST') ?>"
                   class="bxr-view-mode<?= ($arSortGlobal['view'] == 'list') ? ' active' : ''; ?>">
                    <i class="fa fa-th-list"></i>
                </a>
                <a href="<?= $APPLICATION->GetCurPageParam('view=table', array('view')); ?>"
                   title="<?= GetMessage('KZNC_VIEW_TABLE') ?>"
                   class="bxr-view-mode<?= ($arSortGlobal['view'] == 'table') ? ' active' : ''; ?>">
