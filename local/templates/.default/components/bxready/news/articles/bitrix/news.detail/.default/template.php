...
<? if (in_array("DATE_ACTIVE_FROM", $arParams["DETAIL_FIELD_CODE"]) || in_array("DATE_ACTIVE_TO", $arParams["DETAIL_FIELD_CODE"])): ?>
    <div class="date-news">
        <? if (in_array("DATE_ACTIVE_FROM", $arParams["DETAIL_FIELD_CODE"])): ?><?= $arResult["ACTIVE_FROM"] ?><? endif; ?>
        <? if (in_array("DATE_ACTIVE_TO", $arParams["DETAIL_FIELD_CODE"])): ?> / <?= $arResult["DATE_ACTIVE_TO"] ?><? endif; ?>
    </div>
<? endif; ?>
...
<? else: ?>
    ...
    <script type="text/javascript">
        Article.run({
            ajaxUrl: "<?= $this->__folder ?>/ajax.php"
        })
    </script>
    ...
    <? if (count($arResult['PROPERTIES']['PRODUCT_LIST']['VALUE']) > 0): ?>
        <?
        global $arrFilter;
        $arrFilter['ID'] = $arResult['PROPERTIES']['PRODUCT_LIST']['VALUE'];
        $APPLICATION->IncludeComponent(
