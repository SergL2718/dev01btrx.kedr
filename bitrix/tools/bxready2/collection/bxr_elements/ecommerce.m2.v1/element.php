...
$showCatalogQty = ($arElementParams["SHOW_MAX_QUANTITY"] != "N") ? true : false;
    ?><div class="bxr-m2-ecommerce-v1<?=' '.$arElementParams['BXR_PRESENT_SETTINGS']['BXREADY_ELEMENT_ADDCLASS']?>" data-uid="<?=$UID?>" data-resize="1" id="<?=$arItemIDs["ID"]?>">
        ...
        <div class="bxr-element-container">   
            ...
            var <?=$strObName; ?> = new JCCatalogECommerce(<?=CUtil::PhpToJSObject($arJSParams, false, true); ?>);
            </script>
        </div>
    </div><?
include 'epilog.php';
...
<script>
    $(document).ready(function(){
        bxrM2EcommerceV1.init('<?=($arElementParams['LAZY_LOAD']) ? 'true' : 'false'?>');
    });
</script>